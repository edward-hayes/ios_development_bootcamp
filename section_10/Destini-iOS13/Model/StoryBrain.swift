...
struct StoryBrain {
    ...
    mutating func nextStory(choice: String) {
        let currentStory = self.stories[storyStep]
        if choice == currentStory.choice1 {
            storyStep = currentStory.choice1Destination
        } else if choice == currentStory.choice2 {
            storyStep = currentStory.choice2Destination
        }
    }
    
    func getCurrentStory() -> Story {
        return self.stories[storyStep]
    }
}
